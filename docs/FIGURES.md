...
| `fig_v44_mitogen_sensitivity.pdf` | `fig_v44_mitogen_sensitivity.py` | **mitogen sensitivity in Hh-activity (Gli1) units**, probing BELOW the division threshold: titrate Hh from the floor up via GDC0449 (Smo inhibitor) at saturating SHH, ± the EZH2⊣CyclinD1 feedback. (A) CyclinD1 transcript+protein vs Gli1 — feedback represses; (B) proliferation vs Gli1 — the feedback shifts the threshold (Gli1 0.0012→0.0073, ~6×) but **both thresholds sit right against the floor** (GNP physiological Gli1~0.05, MB~0.13 well above), i.e. the basal CyclinD1 floor is too close to threshold for a usable sub-threshold range. Caches the sweep; `--fresh`. (~8 min) |
