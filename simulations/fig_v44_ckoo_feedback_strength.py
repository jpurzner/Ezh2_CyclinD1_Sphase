...
F0s = [0.05, 0.15, 0.233, 0.35, 0.55, 0.85]
fold_m, base_m = [], []
for F0 in F0s:
...
ax[0].axhline(FOLD_DEFAULT, color='#2c3e50', ls=':', lw=1.8, label=f'AUM model ({FOLD_DEFAULT:.1f}x)')
...
ax2.plot(F0s, fold_m, '#2c3e50', lw=2.4, marker='o', label='de-repression fold (cKO/EZH2i readout)')
...
fig.suptitle('Is the model CyclinD1 de-repression supported by the Ezh2 cKO? (accounting for incomplete cKO)',
