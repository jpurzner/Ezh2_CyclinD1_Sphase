...
f0_prc2 -> 1.0, the H3K27me3 mark cannot repress) in GNP+SHH and Ptch+/- MB contexts. Panels:
...
CTX = {
    ...
    'Ptch+/- MB':   dict(SHH=0.5, Ptch1_copy_number=0.1, MYCN_amplification=2.8, p16=0.15, p18=1.5, kSyP21=0.004),
}
...
def run(ctx, feedback=True):
    p = dict(PARAMS)
    if not feedback:
        p["f0_prc2"] = 1.0                    # ablate H3K27me3 -> CyclinD1 repression (R==1)
