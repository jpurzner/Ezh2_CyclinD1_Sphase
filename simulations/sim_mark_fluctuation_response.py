...
DT = CHUNK / (NP - 1)
PART = 0.30
DWELL_CUT = 2.0                                          # h; a p27-high pre-S run >= this that later re-divides = TRANSIENT G0
...
def _work(task):
    ...
    z0 = np.zeros(NB); NULL = (T_h, f0, z0, z0.copy(), z0.copy(), z0.copy(), z0.copy(), z0.copy(), z0.copy())
    ...
    t = 0.0; TREC = max(N_PER * T, 18000.0)              # >= 300 h recorded so transient-vs-arrest is well-determined even for fast drives
    while t < TREC:
    ...
    # run-based TRANSIENT G0 (state 1) vs terminal ARREST (state 2): a >=2h g0 run followed by a later division
    state = np.zeros(len(DNA), np.int8)
    ...
    tg0s = np.array([np.sum((bi == b) & (state == 1)) for b in range(NB)], float)
    arrs = np.array([np.sum((bi == b) & (state == 2)) for b in range(NB)], float)
    divh = np.zeros(NB)
    if len(div):
        dbi = np.clip(np.digitize(PH[div], edges) - 1, 0, NB - 1)
        for b in dbi: divh[b] += 1
    return (T_h, f0, divh, cnt, mks, cds, g0s, tg0s, arrs)
...
if __name__ == '__main__':
    ...
    if FRESH or not os.path.exists(CACHE):
        ...
        for f0, tag in [(0.233, 'W'), (1.0, 'N')]:
            ...
            tg0 = np.full((len(T_DRIVE_H), NB), np.nan); arr = np.full((len(T_DRIVE_H), NB), np.nan)
            for ti, T_h in enumerate(T_DRIVE_H):
                ...
                CDs = np.sum([r[5] for r in sub], 0); G0s = np.sum([r[6] for r in sub], 0); TGs = np.sum([r[7] for r in sub], 0); ARs = np.sum([r[8] for r in sub], 0)
                ...
                tg0[ti] = np.where(C > 0, TGs / C, np.nan); arr[ti] = np.where(C > 0, ARs / C, np.nan)
            store[f'{tag}_rate'] = rate; store[f'{tag}_mk'] = mk; store[f'{tag}_cd'] = cd; store[f'{tag}_g0'] = g0
            store[f'{tag}_tg0'] = tg0; store[f'{tag}_arr'] = arr
    ...
    # ================= Figure 1: CyclinD1 / commitment response (mark SCALES, does not low-pass filter) =================
    f1, x1 = plt.subplots(2, 2, figsize=(15, 11))
    ...
    for a, ti, ttl in [(x1[0, 0], ifast, f'(A) FAST (T={T[ifast]:.0f} h): CyclinD1 tracks Hh; WITH mark lower mean, same relative swing'),
                       (x1[0, 1], islow, f'(B) SLOW (T={T[islow]:.0f} h): CyclinD1 tracks the drive (both)')]:
        _hh(a)
        ...
        a.set_xlabel('drive phase (deg)'); a.set_ylabel('cycle-averaged CyclinD1'); a.set_title(ttl, fontweight='bold', fontsize=10.5); a.legend(fontsize=9, loc='upper left'); a.grid(alpha=0.15)
    a = x1[1, 0]
    for tag, col, nm in [('W', '#8b1a1a', 'WITH mark'), ('N', '#3b7bbf', 'WITHOUT mark')]:
        a.plot(T, [fund(z[f'{tag}_cd'][ti]) for ti in range(len(T))], '-o', color=col, lw=2.7, ms=6, label=f'{nm} — CyclinD1')
        a.plot(T, [fund(z[f'{tag}_g0'][ti]) for ti in range(len(T))], ':s', color=col, lw=2.0, ms=5, alpha=0.8, label=f'{nm} — G0/commitment')
    a.set_xscale('log'); a.set_xlabel('drive period (h) — faster ← → slower'); a.set_ylabel('modulation index (0=buffered, high=tracks)')
    a.set_title('(C) CyclinD1 relative swing ≈ equal & flat (mark SCALES, no low-pass);\nG0/commitment low-pass-filtered but ~EQUAL → buffering is the cell cycle, NOT the mark', fontweight='bold', fontsize=9.5); a.legend(fontsize=7.5, loc='upper left'); a.grid(alpha=0.15, which='both')
    a = x1[1, 1]
    a.plot(T, [(np.nanmax(z['W_mk'][ti]) - np.nanmin(z['W_mk'][ti])) for ti in range(len(T))], '-o', color='#8e44ad', lw=2.7, ms=6, label='H3K27me3 mark amplitude (WITH mark)')
    a.set_xscale('log'); a.set_xlabel('drive period (h) — faster ← → slower'); a.set_ylabel('mark (Mk) peak-to-trough over drive')
    a.set_title('(D) The mark is a slow integrator: its own oscillation is tiny for FAST\ndrives (can\'t follow), grows for SLOW — too small to buffer CyclinD1', fontweight='bold', fontsize=10.5); a.legend(fontsize=9); a.grid(alpha=0.15, which='both')
    f1.suptitle(f'CyclinD1 / commitment response to Hh FLUCTUATIONS — WITH vs WITHOUT H3K27me3 (Hh0={HH0}±{AMP} sine, N={Nc}/cond)', fontsize=12, fontweight='bold', y=1.0)
    plt.tight_layout(); plt.savefig('simulations/sim_mark_fluctuation_response.png', dpi=150, bbox_inches='tight'); plt.savefig('simulations/sim_mark_fluctuation_response.pdf', bbox_inches='tight'); plt.close(f1)

    # ================= Figure 2: TRANSIENT G0 response =================
    ...
    def phase_panel(a, ti, ttl):
        ...
        a2.set_ylabel('Hh drive (grey)', color='#888'); a2.set_ylim(0, 1.1)
        for tag, col, nm in [('W', '#8b1a1a', 'WITH mark'), ('N', '#3b7bbf', 'WITHOUT mark')]:
            a.plot(np.degrees(ph), z[f'{tag}_tg0'][ti], '-o', color=col, lw=2.6, ms=5, label=nm)
        a.set_xlabel('drive phase (deg)'); a.set_ylabel('transient-G0 time-occupancy')
        a.set_title(ttl, fontweight='bold', fontsize=11.5); a.legend(fontsize=9, loc='upper right'); a.grid(alpha=0.15)

    phase_panel(ax[0, 0], ifast, f'(A) FAST fluctuation (T={T[ifast]:.0f} h): transient-G0 over one drive cycle\n(grey=Hh; peaks at LOW Hh)')
    phase_panel(ax[0, 1], islow, f'(B) SLOW fluctuation (T={T[islow]:.0f} h): transient-G0 swings fully with the drive')

    # (C) transient-G0 modulation index vs drive period — the low-pass, WITH vs WITHOUT
    a = ax[1, 0]
    for tag, col, nm in [('W', '#8b1a1a', 'WITH mark'), ('N', '#3b7bbf', 'WITHOUT mark')]:
        mod = np.array([fund(z[f'{tag}_tg0'][ti]) for ti in range(len(T))])
        a.plot(T, mod, '-o', color=col, lw=2.7, ms=6, label=nm)
    a.set_xscale('log'); a.set_xlabel('drive period (h)  — faster ← → slower'); a.set_ylabel('transient-G0 modulation index (0=buffered, high=tracks Hh)')
    a.set_title('(C) Does transient-G0 track the fluctuation? Low at FAST drives\n(buffered) → rises for slower drives. Compare WITH vs WITHOUT', fontweight='bold', fontsize=11); a.legend(fontsize=9); a.grid(alpha=0.15, which='both')

    # (D) MEAN transient-G0 (solid) & arrest (dashed) occupancy vs drive period — WITH vs WITHOUT
    a = ax[1, 1]
    for tag, col, nm in [('W', '#8b1a1a', 'WITH mark'), ('N', '#3b7bbf', 'WITHOUT mark')]:
        a.plot(T, [np.nanmean(z[f'{tag}_tg0'][ti]) for ti in range(len(T))], '-o', color=col, lw=2.7, ms=6, label=f'{nm} — transient-G0')
        a.plot(T, [np.nanmean(z[f'{tag}_arr'][ti]) for ti in range(len(T))], '--s', color=col, lw=1.8, ms=4, alpha=0.7, label=f'{nm} — arrest')
    a.set_xscale('log'); a.set_xlabel('drive period (h)  — faster ← → slower'); a.set_ylabel('mean time-occupancy over the drive')
    a.set_title('(D) MEAN transient-G0 (solid) & arrest (dashed) under fluctuations\nWITH vs WITHOUT — does the mark change how much time cells spend in G0?', fontweight='bold', fontsize=11); a.legend(fontsize=8); a.grid(alpha=0.15, which='both')

    fig.suptitle(f'TRANSIENT G0 response to Hh FLUCTUATIONS — WITH vs WITHOUT H3K27me3 repression (Hh0={HH0}±{AMP} sine, calibrated+mu+partition, N={Nc}/cond)',
                 fontsize=12, fontweight='bold', y=1.0)
    plt.tight_layout()
    plt.savefig('simulations/sim_mark_fluctuation_transientg0.png', dpi=150, bbox_inches='tight')
    plt.savefig('simulations/sim_mark_fluctuation_transientg0.pdf', bbox_inches='tight')
    plt.close()
    for tag, nm in [('W', 'WITH   '), ('N', 'WITHOUT')]:
        print(f'{nm} transient-G0 modulation by drive period {list(T)} =', np.round([fund(z[f"{tag}_tg0"][ti]) for ti in range(len(T))], 3))
        print(f'{nm} mean transient-G0 occupancy               =', np.round([np.nanmean(z[f"{tag}_tg0"][ti]) for ti in range(len(T))], 3))
    print('Saved sim_mark_fluctuation_transientg0.png')
