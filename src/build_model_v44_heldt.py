...
MITOSIS_BLOCK = """
  ...
  f_commit_carry = 0.3721522796032151;   // Feature C (Spencer carryover): fraction of commitment state (phospho-Rb, CyclinE, low-p27) inherited across division. 0 = legacy hard-reset (every G1 crashes E2f -> looks like arrest); >0 lets a committed daughter keep E2f in G1 (immediate re-entry) while uncommitted daughters still reset toward G0.
...
GROWTH_BLOCK = """
  ...
  M_commit = 1.1165839122941084;        // critical cell size for COMMITMENT (G0->G1; Skp2-p27 feedforward fires) ->
...
EZH2_CORE_BLOCK = """
  ...
  kEZbas = 0.0004018457823636438; kEZE2f = 0.005750107942931733; K_E2f_EZ = 0.28261858260369177; Kez_cd = 3.535324963896047;   // EZH2-stability re-fit (was 0.00027/0.022): co-fit with faster kDeEZ against all 9 EZH2-coupled validation targets
  kEZbas_Cd = 0.0003820644229136867;      // mitogen-dose-scaled but CYCLE-FLAT baseline transcription: carries the MB/GNP dose
...
def build_model_v44(hu=None, with_ezh2=True, with_hh=True, with_growth=True,
    ...
    m = m.replace("kSyDna = 0.0093;", "kSyDna = 0.03795827978844792;")
    ...
    if with_h3k27_dilution and with_ezh2 and not with_h3k27_memory:
        ...
        shared = (
            ...
            "\n  del_mk = 0.002063132490889657; k_jmjd3_gli = 0.1093914222600271;   // basal H3K27me3 turnover + Gli->Jmjd3/Kdm6b ACTIVE eraser (Shi 2014 ncomms6425): WRITER=EZH2(cycle) vs ERASER=Gli(mitogen) race (optimize_prc2 2026-07-10)"
        ...
        if with_prc2:
            ...
            mk = shared + (
                "\n  a0_prc2 = 0.0013921398658893584; a_rw_prc2 = 0.001514259806859603; g_prc2 = 0.09185109324427426;   // PRC2 recruitment: accessory (mark-indep, sequence/SUZ12) + H3K27me3 read-write (EED); nascent-tx eviction (optimize_prc2 2026-07-10)"
                "\n  K_prc2 = 0.0010603199882607901; n_prc2 = 1.643403778027013; f0_prc2 = 0.19351965914833702;   // CyclinD1 repression Hill on PRC2 OCCUPANCY + leaky floor (Pol II retained)"
